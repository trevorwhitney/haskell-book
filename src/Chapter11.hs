...
buttonContainsChar char (Button digit possibleChars) = char `elem` possibleChars
...
calculatePresses :: Button -> Char -> (Digit, Presses)
calculatePresses (Button digit possibleChars) char = (digit, y) where
  x = fromJust $ elemIndex char possibleChars
  y = x + 1
...
phone = DaPhone [two, three, four, five, six, seven, eight, nine]
